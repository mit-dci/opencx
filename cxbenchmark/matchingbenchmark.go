...
func PlaceAndFill(client *benchclient.BenchClient, user1 string, user2 string, pair string, howMany int) (numRuns int) {
	for pr := 0.1; pr < 1; pr = pr + 0.1 {
		...
		go client.OrderAsync(user1, "buy", pair, 1000, pr, bufErrChan)
		go client.OrderAsync(user2, "sell", pair, 1000, pr, bufErrChan)
		go client.OrderAsync(user1, "sell", pair, 1000, pr, bufErrChan)
		go client.OrderAsync(user2, "buy", pair, 1000, pr, bufErrChan)
