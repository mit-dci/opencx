...
// NewListener returns a new net.Listener which enforces the cxnoise scheme
...
func NewListener(localStatic *koblitz.PrivateKey, port int) (*Listener,
	...
	for i := 0; i < defaultHandshakes; i++ {
		cxnoiseListener.handshakeSema <- struct{}{}
	}

	go cxnoiseListener.listen()

	return cxnoiseListener, nil
}
...
func (l *Listener) listen() {
	for {
		...
		go l.doHandshake([]byte("opencx"), conn)
	}
}

// doHandshake asynchronously performs the cxnoise handshake, so that it does
...
func (l *Listener) doHandshake(prologue []byte, conn net.Conn) {
	...
	cxnoiseConn := &Conn{
		conn:  conn,
		noise: NewNoiseMachine(false, prologue, l.localStatic),
	}
	...
	if _, err := io.ReadFull(conn, actOne[:]); err != nil {
		cxnoiseConn.conn.Close()
		l.rejectConn(err)
		return
	}
	if err := cxnoiseConn.noise.RecvActOne(actOne); err != nil {
		cxnoiseConn.conn.Close()
		l.rejectConn(err)
		return
	}
	...
	actTwo, err := cxnoiseConn.noise.GenActTwo()
	if err != nil {
		cxnoiseConn.conn.Close()
		l.rejectConn(err)
		return
	...
	if err := cxnoiseConn.noise.RecvActThree(actThree); err != nil {
		cxnoiseConn.conn.Close()
		l.rejectConn(err)
		return
	}
	...
	l.acceptConn(cxnoiseConn)
}

// maybeConn holds either a cxnoise connection or an error returned from the
// handshake.
...
// incoming connections are authenticated via the three act cxnoise
...
func (l *Listener) Accept() (net.Conn, error) {
	...
	case <-l.quit:
		return nil, errors.New("cxnoise connection closed")
	}
}
