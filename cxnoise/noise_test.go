...
func makeListener() (*Listener, string, error) {
	// First, generate the long-term private keys for the cxnoise listener.
...
func establishTestConnection() (net.Conn, net.Conn, func(), error) {
	...
	if err != nil {
		logging.Errorf("Could not make listener")
	...
	if err != nil {
		logging.Errorf("Error getting remote privkey")
		return nil, nil, nil, err
	}
	...
	go func() {
		remoteConn, err := Dial(remotePriv, netAddr, []byte("opencx"), net.Dial)
		if err != nil {
			logging.Error(err)
		}
...
// The test passes if real cxnoise dialer connects while the others are
// stalled.
func TestConcurrentHandshakes(t *testing.T) {
	...
	// Now, construct a new private key and use the cxnoise dialer to
	...
	// This connection should be accepted without error, as the cxnoise
...
func TestBolt0008TestVectors(t *testing.T) {
	...
	// our test.
	initiator := NewNoiseMachine(true, []byte("opencx"), initiatorPriv, initiatorEphemeral)
	responder := NewNoiseMachine(false, []byte("opencx"), responderPriv, responderEphemeral)
	...
	expectedActOne, err := hex.DecodeString("01036360e856310ce5d294e" +
		"8be33fc807077dc56ac80d95d9cd4ddbd21325eff73f77e6bbdd15877" +
		"6b1cac22f9b2a6f9b1fd")
	if err != nil {
	...
	expectedActTwo, err := hex.DecodeString("0102466d7fcae563e5cb09a0" +
		...
		"85d1f568b4c2b5048e8534b873319f3a8daa612b469132ec7f70d5f8d07" +
		"2aad9a1be4141ccd3bdc7376")
	if err != nil {
	...
	expectedActThree, err := hex.DecodeString("018ac8fc232a47aa6fa5c51" +
		"b3b72c5824018e9d92f0840a5eada20f3b00d66a0e4c99f51242f270" +
		"97320ecc1ae95b17571ba8338a0ca481f6240bd15d199b6de0270")
	if err != nil {
