...
const (
	...
	// waiting for data payloads during the various acts of cxnoise. If
...
// Machine is a state-machine which implements cxnoise: an
// Authenticated-key Exchange in Three Acts. cxnoise is derived from the Noise
...
// NewNoiseMachine creates a new instance of the cxnoise state-machine. If
// the responder (listener) is creating the object, then the remotePub should
// be nil. The handshake state within cxnoise is initialized using the ascii
// string "opencx" as the prologue. The last parameter is a set of variadic
// arguments for adding additional options to the cxnoise Machine
// initialization.
func NewNoiseMachine(initiator bool, prologue []byte, localStatic *koblitz.PrivateKey,
	options ...func(*Machine)) *Machine {

	handshake := newHandshakeState(initiator, prologue, localStatic)
